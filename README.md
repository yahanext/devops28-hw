...
```
использует обе опции ignorespace и ignoredups (не сохраняет строки начинающиеся с символа пробел и не сохраняет строки,
совпадающие с последней выполненной командой
```

1. В каких сценариях использования применимы скобки `{}`, на какой строчке `man bash` это описано?
```
Такие скобки позволяют объеденить несколько операторов в один составной и в отличии от круглых скобок выполнятся будет
в контексте текущей оболочке, а не в отдельном подпроцессе. Описано в 343 строке man bash
```
