...
```
Это все прекрасно, но из за санкций пришлось городить l2tp/ipsec vpn 
адреса vpn серверов и инструкцию по подключению нашел на www.vpngate.net
При регистрации указал страну - Армения, в дальнейшем все успешно.
Работает медленно, можно успеть кофе сварить.
```
...
```
Описывается на строке 1185 Manual page bash
параметр HISTFILESIZE - максимальное кол-во строк в файле bash_history
задать можно так
...
```
использует обе опции ignorespace и ignoredups (не сохраняет строки начинающиеся с символа пробел и не сохраняет строки, совпадающие с последней выполненной командой
```

1. В каких сценариях использования применимы скобки `{}`, на какой строчке `man bash` это описано?
```
Такие скобки позволяют объеденить несколько операторов в один составной и в отличии от круглых скобок выполнятся будет в контексте текущей оболочке, а не в отдельном подпроцессе. Описано в 343 строке man bash
```
...
```
touch test{1..100000}
300000 не получится т.к есть ограничение на длину аргумента MAX_ARG_STRLEN
```

5. В man bash поищите по `/\[\[`. Что делает конструкция `[[ -d /tmp ]]`?
```
возвращет 0 т.к каталог существует
```

7. Сделайте так, чтобы в выводе команды `type -a bash` первым стояла запись с нестандартным путём, например, bash is... Используйте знания о просмотре существующих и создании новых переменных окружения, обратите внимание на переменную окружения PATH.
...
```
Создаем каталог mkdir /tmp/bash
делаем символическую ссылку ln -s /bin/bash /tmp/bash/bash
Прописываем новый путь PATH=/tmp/bash/:$PATH
...
```
at - предназначена для выполнения задания один раз в определнное время
batch - система сама решает когда запустить команду, при низкой загрузке
```
